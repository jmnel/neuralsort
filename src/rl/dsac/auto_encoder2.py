...
class ModelA(nn.Module):
    ...
    def __init__(self):
        ...
        self.lstm_3 = nn.LSTM(input_size=HIDDEN_SIZE,
                              hidden_size=HIDDEN_SIZE,
                              num_layers=NUM_LAYERS,
                              batch_first=True,
                              bias=LSTM_BIAS)
        ...
        self.hidden_cell_2 = (torch.zeros(NUM_LAYERS, BATCH_SIZE_TRAIN, HIDDEN_SIZE).to(DEVICE),
                              torch.zeros(NUM_LAYERS, BATCH_SIZE_TRAIN, HIDDEN_SIZE).to(DEVICE))
        self.hidden_cell_3 = (torch.zeros(NUM_LAYERS, BATCH_SIZE_TRAIN, HIDDEN_SIZE).to(DEVICE),
                              torch.zeros(NUM_LAYERS, BATCH_SIZE_TRAIN, HIDDEN_SIZE).to(DEVICE))
        ...
        self.fc_3 = nn.Linear(in_features=HIDDEN_SIZE * SEQ_LEN, out_features=HIDDEN_SIZE * SEQ_LEN // pow(2, 2))
        self.fc_4 = nn.Linear(HIDDEN_SIZE * SEQ_LEN // pow(2, 2), 1)
...
#        x = np.vstack((ts, prices, sizes))
        x = prices

        x = x.reshape((BATCH_SIZE_TRAIN, x.shape[1], LSTM_FEATURES))
...
#        plt.show()
        ...
        ax3.plot(np.arange(0, min(epoch + 1, 50)), train_losses[-50:])
        ax3.plot(np.arange(0, min(epoch + 1, 50)), validate_losses[-50:])
