...
class Gan:
    ...
    def train(self,
        ...
        for idx, (x_example, ts_example) in enumerate(data_loader):
            ...
            x_2 = x_example[0].numpy()
            foo = np.zeros(4000)
            foo[0] = ts_example[0, 0]
            for i in range(1, 4000):
                foo[i] = foo[i - 1] * np.exp(x_2[i])
...
#            foo = np.exp(foo)
            ...
            axs[0].plot(x_example[0], lw=0.4)
            axs[1].plot(ts_example[0], lw=0.4)
            axs[2].plot(foo, lw=0.4)
            seaborn.distplot(x_example[0], bins=1000, ax=axs[3])

            mu = torch.mean(x_example[0]).item()
            var = torch.var(x_example[0]).item()
            krt = stats.kurtosis(x_example[0])
            skew = stats.skew(x_example[0])

            print(f'kurtosis: {krt}, skew: {skew}')
